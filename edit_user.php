...
// all error checking and inserting into database
      if (empty($emailError) && empty($last_nameError) && empty($passwordError)) {
       ...
       // checking if the email is new and send confirmation email
        if ($_SESSION['edit_email'] !=$email) {

          $sql2="UPDATE users SET last_name='$last_name', first_name='$first_name', email='$email', password='$pass', user_type='$user_type', salt='$salt', email_activated='0', updated_at= NOW() WHERE id='$user_id'";

                  if (mysqli_query($db,$sql2)) {
                    $message= "<div class='btn btn-success fade-in flash'>You have updated $last_name $first_name successfully with email confirmation!</div>";
                    
                    $id = $user_id;
                    send_mail_reconfirm ($email, $last_name, $id, $salt);
                  }

                   
              }else{

                $sql2="UPDATE users SET last_name='$last_name', first_name='$first_name', email='$email', password='$pass', user_type='$user_type', salt='$salt', updated_at= NOW() WHERE id='$user_id'";

                    if(mysqli_query($db, $sql2)){                                           
                      
                      $message= "<div class='btn btn-success fade-in flash'>You have updated $last_name $first_name successfully!</div>";
                    }
